...
public class Map_LayerPanel : MonoBehaviour
{
    ...
    public void Init(ExpeditionManager.Room[] r,int lc,ExpeditionManager em,InfoText it)
    {
        layerCount = lc;
        layerText.text = layerCount.ToString();

        for(int i = 0; i < 5; i++) { roomButtons[i].Init(r[i], new Vector2Int(layerCount, i),em,it); }
    }
