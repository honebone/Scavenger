           ...
           [Header("全部％で")] 
            ...
            public string GetInfo(bool refStatus=false, Character.CharacterStatus status=new Character.CharacterStatus())
            {
               ...
               if (refStatus) mod = ToStatuMod(status);
                string info = "";
                if (maxHP > 0) { info += $"{NL()}・{"maxHP".ToSpr_withName()}の{(maxHP + "％").ColorStr(Definer.colorRef.emphasize)}{(refStatus ? $"({mod.maxHP_int})" : "")}"; }
                if (ATK > 0) { info+=$"{NL()}・{"ATK".ToSpr_withLink()}の{(ATK + "％").ColorStr(Definer.colorRef.emphasize)}{(refStatus ? $"({mod.ATK_int})" : "")}"; }
                if (INT > 0) { info+=$"{NL()}・{"INT".ToSpr_withLink()}の{(INT + "％").ColorStr(Definer.colorRef.emphasize)}{(refStatus ? $"({mod.INT_int})" : "")}"; }
           ...
           if(actionStatus.abilityEffect) target.BecomeAbilityTarget(actionStatus.actionOwner);
            ...
            if (!targetStatus.dead)//対象が生きているときのみ、効果発動
            {
                ...
                if (attackHit && target.CheckAlive())
                {
                    ...
                    if (actionsStatus[i].DoesAddShield())//シールド
                    {
