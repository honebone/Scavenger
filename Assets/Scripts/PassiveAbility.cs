...
public class PassiveAbility : MonoBehaviour
{
    ...
    {
        infoText.AddLogText($"<{character.CharaStatus().charaName}‚Ì{GetPAName()}>F{str}");
        character.SetDamageText($"{GetPAName()}F{str}", Definer.colorRef.currentState);
    }

    /// <summary>w’è‚µ‚½ğŒ‚É‡’v‚·‚é‘ÎÛ‚ğ’T‚µ‚ÄEnqueue ÀÛ‚ÉEnqueue‚µ‚½‚©‚ğ•Ô‚·</summary>
